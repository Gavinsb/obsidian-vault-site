...
- **2026-09-24 — Wave 3 complete.** `Editor.tsx` wraps `AtomicCodeMirrorEditor` (`value`/`onChange`/`readOnly`/`path`/`documentId`/`onSave`) and replaces the `<textarea>` in `DocView` with the draft/baseline/ETag/conflict state machine untouched. A CM6 completion source calls `GET /api/completions` for note/tag/heading/blockref/callout, reusing `findAutocompleteTrigger`/`applyAutocomplete` for `[[` and `#`, adding `[[Note#` (heading), `[[Note#^` (blockref) and `> [!` (callout), with 200 ms debounce, stale-result guards and `filter:false` so server ranking is preserved; code suppression now uses the Lezer tree so fences nested in lists/quotes are covered. 13 new tests. **Pre-existing bug fixed:** the root `@lezer/common` pin (1.2.3) forced a second nested copy (1.5.2) under `@codemirror/language`, which crashed CM6's tree highlighter on every mount (`tags is not iterable`, no syntax highlighting + console error); raised the pin to 1.5.2 (and `@lezer/highlight` to 1.2.4) so it dedupes, and added `happy-dom` as a devDependency (the editor suite needs it; it had only been installed with `--no-save`). Gates: lint clean; Vitest **171/171 across 24 files**; production build passed; QA 27/27.
