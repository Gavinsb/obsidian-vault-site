...
**Status:** RELEASED 2026-09-24 — all nine waves (0–8) implemented, verified, deployed and pushed (release commit `bb02b28`).
**Sprint:** S7
...
- **2026-09-24 — Wave 8 complete — S7 RELEASED.** `EditingHelp.tsx` documents the shipped editor (slash menu and its exact scaffold list, toolbar marks, gutter and drag/drop, multi-block single-step undo, Properties with read-only `updated`, embeds/block refs/masked preview, and the Agent Console with its transition, ID, reviewer-feedback and Save-gate rules); `docs/sprints.md` carries the full S7 entry (13 items, nine waves, commit trail); the stale `wysiwyg-editor-spec.md` status line now reads implemented. Release gate: `npm run lint` clean, Vitest **278/278 across 30 files**, production build passed, `npm run qa` 27/27, `git diff --check` clean. Deploy: rebuilt and restarted `kv-microsite.service` (active/enabled, clean startup, 63 documents indexed); local `127.0.0.1:18790` and the public tunnel `https://items-lakes-dream-keyboards.trycloudflare.com` both `200`, `/api/completions` and `/api/note-preview` public, `/api/agent/ids` `401` anonymously with no agent content. Committed as `bb02b28` with the Gavinsb identity, pushed to `master`, HEAD parity confirmed. Known non-blocking follow-ups: `[[Note#Heading]]` navigation drops the heading; block anchors are click-only (no Enter/Space); `frontmatter.ts` quotes ISO timestamps on server writes; bulk delete/duplicate helpers exist and are tested but are not yet bound to a UI gesture; `EditingHelp` names the CM6 engine once.
