...
async function main() {
  ...
  // 2. Boot the server.
  const child = spawn(process.execPath, ['--import', 'tsx', 'src/server/server.ts', cfgPath], {
    cwd: PROJ,
  ...
  check('2. Configured vault loads', ov.status === 200 && ov.body.docCount >= 6, `docCount=${ov.body.docCount}`);

  // 4. Markdown renders — GET a doc and confirm raw content present.
  const doc = await j('/docs/' + encodeURIComponent('01 Concepts/ai/mcp/MCP Security.md'));
  check('3. Markdown content fetch', doc.status === 200 && doc.body.content.includes('# MCP Security'));
  check('4. Wiki-links parse (outgoing listed)', Array.isArray(doc.body.outgoing) && doc.body.outgoing.length > 0);
  check(
    '13. Broken/unresolved link flagged',
    doc.body.outgoing.some((o) => o.target.includes('Does Not Exist') && !o.resolved)
  );

  // 5. Backlinks.
  const agent = await j('/docs/' + encodeURIComponent('01 Concepts/ai/Agent Architecture.md'));
  check(
    '5. Backlinks correct',
    Array.isArray(agent.body.backlinks) && agent.body.backlinks.some((b) => b.sourceRelPath.includes('MCP Security'))
  );

  // 6. Search.
  const s = await j('/search?q=agent');
  check('6. Search finds documents', s.status === 200 && s.body.results.some((r) => r.title.toLowerCase().includes('agent')));

  // 7. Graph.
  const g = await j('/graph');
  check('7. Graph relationships correct', g.status === 200 && g.body.nodes.length >= 3 && g.body.edges.length >= 1);

  // 8. Edit a page via API changes source Markdown.
  const target = '03 Notes/Orphan Note.md';
  const before = await j('/docs/' + encodeURIComponent(target));
  const edited = before.body.content + '\n\nAdded by QA.\n';
  const putRes = await fetch(BASE + '/docs/' + encodeURIComponent(target), {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ content: edited, expectedHash: before.body.meta.contentHash }),
  });
  ...
  const yamlDoc = await j('/docs/' + encodeURIComponent(yamlTarget));
  const { setFrontmatterField } = await import(
    new URL('../../src/shared/frontmatter.ts', import.meta.url).href
  );
  const updatedWithRating = setFrontmatterField(yamlDoc.body.content, 'rating', 4);
  await fetch(BASE + '/docs/' + encodeURIComponent(yamlTarget), {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ content: updatedWithRating, expectedHash: yamlDoc.body.meta.contentHash }),
  });
  const yamlOnDisk = await fs.readFile(path.join(vault, yamlTarget), 'utf8');
  check('9. Editing YAML preserves unrelated properties', yamlOnDisk.includes('type: concept') && yamlOnDisk.includes('status: active'));
  ...
  check('10. Rating writes to frontmatter', rated.status === 200 && rated.body.rating === 4);
  ...
  await fs.writeFile(path.join(vault, '02 Guides', 'Tool Use Loop.md'), '# Tool Use Loop\n\nExternal edit while server running.\n', 'utf8');
  await j('/vault/reindex', { method: 'POST' });
  const extDoc = await j('/docs/' + encodeURIComponent('02 Guides/Tool Use Loop.md'));
  check('11. External change reflected in site', extDoc.body.content.includes('External edit while server running'));

  // 12. New external file appears.
  await fs.writeFile(path.join(vault, '05 Projects', 'Brand New.md'), '# Brand New\n', 'utf8');
  await j('/vault/reindex', { method: 'POST' });
  const newOv = await j('/vault/overview');
  const newSearch = await j('/search?q=Brand%20New');
  check('12. New external file appears', newSearch.body.results.some((r) => r.relPath.includes('Brand New')));

  // 13. Deleted external file disappears.
  await fs.unlink(path.join(vault, '03 Notes', 'Orphan Note.md'));
  await j('/vault/reindex', { method: 'POST' });
  const gone = await j('/docs/' + encodeURIComponent('03 Notes/Orphan Note.md'));
  ...
  check('15. Search index updates', upSearch.body.results.some((r) => r.relPath.includes('Tool Use Loop')));

  // 16. Graph index updates (new node appears).
  const upGraph = await j('/graph');
  check('16. Graph index updates', upGraph.body.nodes.some((n) => n.id.includes('Brand New')));

  // 17. Broken links identified.
  const health = await j('/health');
  check('17. Broken links identified', health.body.issues.some((i) => i.kind === 'broken-link'));

  // 18. Orphans identified.
  check('18. Orphans identified', health.body.issues.some((i) => i.kind === 'orphan'));

  // 19. Recent documents correct.
  const recent = await j('/recent?limit=50');
  check('19. Recent documents correct', recent.status === 200 && recent.body.length >= 5);

  // 20. Recent changes recorded.
  const changes = await j('/changes');
  check('20. Recent changes recorded', changes.status === 200 && changes.body.buckets);

  // 21. Conflict detected instead of silent overwrite.
  const cDoc = await j('/docs/' + encodeURIComponent('05 Projects/AI Agent Notes.md'));
  await fs.writeFile(path.join(vault, '05 Projects', 'AI Agent Notes.md'), '# AI Agent Notes\n\nExternal newer\n', 'utf8');
  const conflictRes = await fetch(BASE + '/docs/' + encodeURIComponent('05 Projects/AI Agent Notes.md'), {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ content: 'my edit', expectedHash: cDoc.body.meta.contentHash }),
  });
  check('21. Edit conflict detected (no silent overwrite)', conflictRes.status === 409);

  // 22. No app cache written into the vault.
  const vaultEntries = await fs.readdir(vault);
  const polluted = vaultEntries.filter((e) => e.startsWith('.kv') || e === 'index.json' || e === '.index');
  check('22. No app cache in the vault', polluted.length === 0, polluted.join(','));
  ...
  const { ObsidianFileSystemVaultProvider } = await import(
    new URL('../../src/server/obsidian-filesystem-provider.ts', import.meta.url).href
  );
  const p2 = new ObsidianFileSystemVaultProvider(vault2, { watch: false });
  const p2docs = await p2.listDocuments();
  await p2.close();
  check('23. Same app works on a second vault', p2docs.length === 1 && p2docs[0].relPath === 'notes/Only.md' );
