...
import type { ObsidianFileSystemVaultProvider } from './obsidian-filesystem-provider.js';
...
export class VaultService {
  ...
  async start() {
    ...
    if (this.config.fileWatching) {
      this.watcherCleanup = this.provider.watch((e) => this.handleWatchEvent(e));
    }
  }

  /** Full rebuild of the index from source (also reconciliation). */
  async reconcile(): Promise<number> {
    this.setSyncState({ state: 'indexing', pendingFiles: 0, detail: 'Scanning vault…' });
    ...
    this.setSyncState({ state: 'indexing', pendingFiles: files.length, detail: `Reading ${files.length} documents…` });
    for (const summary of files) {
      const full = await this.provider.readDocument(summary.relPath);
      if (full) withContent.push({ meta: full.meta, content: full.content });
    }
  ...
  private handleWatchEvent(e: { event: string; relPath?: string; absPath: string }) {
    ...
    this.setSyncState({ state: 'detected', pendingFiles: this.debounceTimers.size + 1, detail: relPath });
    ...
    this.debounceTimers.set(
      ...
      setTimeout(() => {
        ...
        void this.applyWatchEvent(e.event as 'add' | 'change' | 'unlink', relPath).then(() => {
          const pending = this.debounceTimers.size;
          if (pending === 0) this.setSyncState({ state: 'synced', pendingFiles: 0 });
  ...
  private async applyWatchEvent(
    ...
    this.index.upsertNote(full.meta, full.content);
  ...
  async saveDocument(relPath: string, content: string, expectedHash: string | null) {
    const res = await this.provider.saveDocument(relPath, content, { expectedHash });
    if (res.ok) {
      const full = await this.provider.readDocument(relPath);
      if (full) {
        this.index.upsertNote(full.meta, full.content);
