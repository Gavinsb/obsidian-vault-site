...
export function loadConfig(explicitConfigPath?: string): AppConfig {
  ...
  const raw = readJSON<AppConfig>(cfgPath || defaultPath);
  const resolveCfgPath = (value: string | undefined, fallback: string) =>
    path.resolve(root, value || fallback);
  const base: AppConfig = {
    ...raw,
    dataDir: resolveCfgPath(raw.dataDir, "data"),
    indexLocation: resolveCfgPath(raw.indexLocation, "data/index.json"),
    backupDir: resolveCfgPath(raw.backupDir, "data/backups"),
  };
  ...
  if (!fs.existsSync(vaultPath)) {
    ...
    try {
      ...
      try {
        ...
        checks.read = {
          pass: false,
          detail: "Directory is not readable (permissions)",
        };
      ...
      for (const e of all) {
        if (e.isFile() && /\.md$/i.test(e.name)) {
          ...
          try {
            ...
            checks.readMarkdown = {
              pass: true,
              detail: "Markdown is readable",
            };
          } catch {
            checks.readMarkdown = {
              pass: false,
              detail: "Markdown not readable",
            };
      ...
      if (!anyMarkdown) {
        checks.hasMarkdown = {
          pass: true,
          detail:
            "No markdown at root (may exist in subfolders); will attempt deeper scan",
        };
      }
      ...
      try {
        ...
        checks.write = {
          pass: false,
          detail: "Directory is not writable (permissions)",
        };
