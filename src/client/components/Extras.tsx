...
export function HighlyRated() {
  ...
  useEffect(() => {
    api.docs().then((all) => setDocs(all.filter((d) => d.rating !== undefined).sort((a, b) => (b.rating ?? 0) - (a.rating ?? 0)))).catch(() => {});
  }, []);
  return (
    <div className="view">
      <h1>Highly rated</h1>
...
export function FolderView() {
  ...
  useEffect(() => {
    ...
    api.docs().then((all) => setDocs(all.filter((d) => d.folder === current))).catch(() => {});
  }, [current]);
    ...
    <div className="view">
      ...
      <div className="folder-tree">
        {folders.map((f) => (
          ...
          >
            📁 {f.folder || '(root)'} <span className="tag-count">{f.count}</span>
          </button>
        ))}
      </div>
      {current && (
...
export function OrphansView() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const orphans = h.issues.filter((i) => i.kind === 'orphan').map((i) => i.relPath!);
  ...
  return (
    <div className="view">
      ...
      <p className="muted hint">Orphans have no incoming links and no resolved outgoing links.</p>
      {issues.length > 0 && <div className="muted">{issues.length} orphan(s) found.</div>}
    </div>
  );
}
    ...
    <div className="view">
      ...
      <section className="card">
        ...
        <dl className="meta-list">
          ...
          <dt>Path</dt>
          <dd className="mono">{config?.vaultPath}</dd>
          <dt>Rating scale</dt>
      ...
      <section className="card">
        ...
        <div className="theme-toggle">
          {(['dark', 'light', 'system'] as const).map((t) => (
            <button key={t} className={theme === t ? 'primary' : ''} onClick={() => setTheme(t)}>
      ...
      <section className="card">
        <h3>About</h3>
        <p className="muted">
          A local-first microsite over your Obsidian vault. The Markdown files are the source of
          truth; indexes and caches are disposable and stored outside the vault.
        </p>
      </section>
    </div>
  );
